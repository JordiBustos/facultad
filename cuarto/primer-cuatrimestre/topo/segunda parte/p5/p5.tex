...
\begin{proof}
    ...
    Sin embargo, veamos que \( f \) es continua en \( \tau_d \) en \( \R \) y \( \tau_P \) en \( \R^{\N} \). Para ello, dada una sucesión \( {(t_i)}_{i \in \N} \) que converge a \( t \) en \( \tau_d \) entonces \( t_i \to t \) en la topología usual de \( \R \). Por lo tanto, \( f(t_i) = (t_i, t_i, t_i, \ldots) \to (t, t, t, \ldots) = f(t) \) en la topología producto pues cada coordenada converge a \( t \). Por lo tanto, \( f \) es continua.
\end{proof}
...
\end{ejercicio}
\begin{proof}
    Para demostrar que el conjunto de funciones continuas \(C(X, \R)\)
    es denso en el espacio de todas las funciones \(\R^X\) provisto de la topología producto, debemos probar que cualquier entorno abierto básico y no vacío de \(\R^X\) contiene al menos un elemento de \(C(X, \R)\).

    Sea \(U \subseteq \R^X\) un conjunto abierto básico y no vacío en la topología producto. Sabemos que \( U \) es de la forma:
    \[
        U = \{f \in \R^X \mid f(x_i) \in V_i \text{ para } i = 1, \dots, n\}
    \]
    donde \( x_1, \dots, x_n \) son puntos distintos en \( X \), y \(V_1, \dots, V_n\) son conjuntos abiertos y no vacíos en \( \R \).

    Para demostrar la densidad, necesitamos construir una función \(g \in C(X, \R)\) tal que \(g \in U\). Esto equivale a exigir que \(g(x_i) \in V_i\) para todo \({i = 1, \dots, n}\).
    ...
    \begin{enumerate}
        \item \(h_i(x_i) = 1\)
        \item \(h_i \big|_{X \setminus U_i} = 0\)
    \end{enumerate}
    ...
    Finalmente, definimos la función \( g: X \to \R \) como:
    \[
        g(x) = \sum_{i=1}^{n} y_i h_i(x)
    \]
    Analicemos las propiedades de esta función:
    \begin{itemize}
        \item Claramente \( g \) es continua.
        \item \textbf{Pertenece a \(U\):} Evaluemos la función \(g\) en cualquier punto \(x_k\) de nuestro conjunto finito de coordenadas:
              \[
    ...
    Hemos construido una función continua \(g \in C(X, \mathbb{R})\) que cumple simultáneamente con todas las restricciones impuestas por el entorno abierto básico \(U\). Esto prueba que la intersección \(U \cap C(X, \mathbb{R})\) es no vacía, por lo tanto, \(C(X, \R)\) es denso en \(\R^X\).
\end{proof}
...
\end{ejercicio}
\begin{proof}
    Dado \( U \in \tau \), como \( X \) es regular, para cada \( x \in U \) existe un entorno abierto \( V_x \) de \( x \) tal que \( x \in V_x \subseteq \cl{V_x} \subseteq U \). Luego, como tengo una base numerable, existe \( x \in B_{n_x} \subseteq V_x \) para algún \( n_x \in \N \). Entonces, \( x \in \cl{B_{n_x}} \subseteq \cl{V_x} \subseteq U \). Por lo tanto,
    \[
    ...
    Para ver la segunda parte, dado \( U \in \tau \), por la primera parte existe una sucesión \( {(F_n)}_{n \in \N} \) de conjuntos cerrados de \( X \) tal que \( U = \bigcup_{n \in \N} F_n \). Como \( X \) es un espacio regular con base numerable, \( X \) es Lindelöf, y todo espacio regular y Lindelöf es normal.

    Para cada \( n \in \N \), notemos que \( F_n \) y el complemento \( X \setminus U \) son conjuntos cerrados y disjuntos (pues \( F_n \subseteq U \)). Por el Lema de Urysohn, existe una función continua \( f_n : X \to [0, 1] \) tal que \( f_n(F_n) = \{1\} \) y \( f_n(X \setminus U) = \{0\} \).

    Consideremos la función \( f : X \to [0, 1] \) dada por
    \[
    ...
    Además, si \( x \in U \) entonces \( x \in F_{n_0} \) para algún \( n_0 \in \N \), lo que implica que \( f_{n_0}(x) = 1 \). Como todos los términos de la serie son no negativos, \( f(x) \geq \frac{1}{2^{n_0}} > 0 \). Por otro lado, si \( x \notin U \) entonces \( x \in X \setminus U \), lo que implica que \( f_n(x) = 0 \) para todo \( n \in \N \). Por lo tanto, \( f(x) = 0 \).

    En conclusión, \( f \) es la función continua que cumple las condiciones requeridas.
\end{proof}
