...
\begin{definition}\label{def:k4}
    El ciclo \( K_1 \) diremos que es el loop y el ciclo \( K_2 \) es el ciclo de dos vértices con dos aristas.
...
\begin{ejercicio}{9}
    Sea \( G \) un grafo auto-complementario (es isomorfo a su complemento). Probar que \( G \) tiene un vértice de corte si y sólo si \( G \) tiene un vértice de grado 1.
\end{ejercicio}
\begin{proof}
...
\begin{definition}
    Un vértice \( v \) de un grafo \( G \) se dice \textbf{aislado} si \( \deg(v) = 0 \).
\end{definition}
