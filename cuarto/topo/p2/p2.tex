...
\end{ejercicio}
\begin{proof}
    Recordemos que el hecho de que \( X \) sea \( T_1 \) significa que para \( x, y \in X \) distintos, existe \( U \in \mathcal{O}(x) \) tal que \( y \notin U \) y existe \( V \in \mathcal{O}(y) \) tal que \( x \notin V \).
    Supongamos entonces que  \( X \) es \( T_1 \) y veamos que los puntos de \( X \) son cerrados.
    Consideremos \( X - \{ x \} = \bigcup_{y \in X - \{ x \}} V_y \) donde \( V_y \in \mathcal{O}(y) \) es tal que \( x \notin V_y \) y, por lo tanto, \( X - \{ x \} \) es abierto y entonces \( \{ x \} \) es cerrado.
...
\begin{proof}
    ...
    \] queremos ver que existe \( \coint{x}{y} \in \tau_s \) tal que \( \coint{a_n}{b_n} \nsubseteq \coint{x}{y} \quad \forall n \in \N \). En efecto,
    Como \( \beta' \) es numerable la colección de los \( a_n, b_n \) es numerable y no pueden cubrir toda la recta real. De aquí se deduce que debe existir \( x \in \R \) tal que \( x \neq a_n \)
    ...
    Veamos si es Lindeloff, por el \textbf{Ejercicio 2.4.4} del apunte de Demetrio, basta ver si para \( \sigma \subseteq \beta \) un cubrimiento de \( \R \) existe un subcubrimiento numerable de \( \sigma \).
    Dado \( \sigma \) como dijimos, es claro que \( \exists I \) una familia de índices tal que \[
        \sigma = \{ \coint{a_i}{b_i} : i \in I \} \subseteq \beta
    \]
    y \( \bigcup_{i \in I} \coint{a_i}{b_i} = \R \). \textbf{TODO}: ir a la topología usual de \( \R \) quedarse el cubrimiento numerable, ver que los extremos izquierdos que se pierden son numerables y quedarse con más (numerables) intervalos del cubrimiento original.
...
\end{ejercicio}
\begin{proof}
    Supongamos que \( X \) es Lindelöf y veamos que \( f(X) \) también lo es. Sea \( \sigma \) un cubrimiento abierto de \( f(X) \) entonces \( \{ f^{-1}(U) : U \in \sigma \} \) es un cubrimiento abierto de \( X \) y, por ser \( X \) Lindelöf, existe un subcubrimiento numerable \( \{ f^{-1}(U) : U \in \sigma' \} \) con \( \sigma' \subseteq \sigma \).
    Luego, \[
        X = \bigcup_{U \in \sigma'} f^{-1}(U) \implies f(X) = f\left( \bigcup_{U \in \sigma'} f^{-1}(U) \right) = \bigcup_{U \in \sigma'} f(f^{-1}(U)) \subseteq \bigcup_{U \in \sigma'} U
    \]
    Luego, es claro que \( \{ f(U) : U \in \sigma' \} \) es un subcubrimiento numerable de \( f(X) \) contenido en \( \{ f(U) : U \in \sigma \} \). Por lo tanto, \( f(X) \) es Lindelöf.

    Supongamos ahora que \( X \) es separable y veamos que \( f(X) \) también es separable.
    En efecto, dado \( D \subseteq X \) un subconjunto numerable y denso en \( X \) entonces \( \cl{D} = X \) y, por continuidad de \( f \), \( f(\cl{D}) \subseteq \cl{f(D)} \) lo que implica que \( f(X) = f(\cl{D}) \subseteq \cl{f(D)} \) y, por lo tanto, \( f(D) \) es un subconjunto numerable y denso en \( f(X) \). Por lo tanto, \( f(X) \) es separable.
\end{proof}
...
\end{ejercicio}
\begin{proof}
    Supongamos primeramente que \( X \) es \( N_1 \) y veamos que \( f(X) \) también es \( N_1 \). En efecto,
    dado \( y \in f(X) \) existe \( x \in X \) tal que \( f(x) = y \). Además, como \( X \) es \( N_1 \)
    existe \( {\{ B_i \}}_{i \in \N} \) una familia numerable de abiertos \( X \) tal que para cada \( U \in \tau \)
    \( \exists n \in \N \) tal que \( x \in B_n \subseteq U \). Luego, como \( f \) es abierta se satisface que
    \( f(B_n) \) es un abierto de \( Y \). Sea \( V \) un entorno de \( y \) entonces por la continuidad de \( f \) se tiene que \( f^{-1}(V) \)
    es un entorno abierto de \( x \) y existe \( n \in \N \) tal que \( x \in B_n \subseteq f^{-1}(V) \implies y \in f(B_n) \subseteq V \). Por lo tanto, \( f(X) \) es \( N_1 \).

    Supongamos ahora que \( X \) es \( N_2 \) y veamos que \( f(X) \) también es \( N_2 \). En efecto,
    por hipótesis existe \( \beta = \{ B_n : n \in \N \} \) una base numerable de la topología de \( X \).
    Luego, \( \beta' = \{ f(B_n) : n \in \N \} \) es una familia numerable de abiertos de \( Y \). Para ver que \( \beta' \) es una base de la topología de \( f(X) \) basta con notar que dado \( y \in f(X) \) y \( V \) un entorno de \( y \) entonces por continuidad de \( f \) se tiene que \( f^{-1}(V) \) es un entorno abierto de \( x \) y existe \( n \in \N \) tal que \( x \in B_n \subseteq f^{-1}(V) \) y por lo tanto \( y \in f(B_n) \subseteq V \). Por lo tanto, \( f(X) \) es \( N_2 \).
\end{proof}
