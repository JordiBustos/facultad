...
\end{ejercicio}
\begin{proof}
    Sea \( U \) un conjunto abierto y conexo en \( X \). Para cada \( x \in U \) consideremos \( C_x = \{ y \in U : y \overset{arc}{\sim} x \} \) la componente arcoconexa de \( x \) en \( U \) y, si logramos demostrar que es clopen relativo a \( U \), entonces \( C_x = U \) y, por lo tanto, \( U \) sería arcoconexo.
...
Del apunte de Demetrio:

\begin{ejercicio}{13}
    Sea \( (X, \tau) \) un espacio topológico conexo, \( f : X \to \R \) una función continua y \( x, y \in X \) probar que \( f \) toma todos los valores entre \( f(x) \) y \( f(y) \).
\end{ejercicio}
\begin{proof}
    Por el absurdo, supongamos que \( f \) no toma algún valor entre \( f(x) \) y \( f(y) \). Spdg \( f(x) < c < f(y) \) y \( f(z) \neq c \) para todo \( z \in X \).
    Entonces, definimos: \[
        U = f^{-1}((-\infty, c)) \quad \text{y} \quad V = f^{-1}((c, +\infty)).
    \]
    Que son abiertos en \( X \) por la continuidad de \( f \). Además, \( U \cap V = \varnothing \) y \( U \cup V = X \), lo que contradice la conexidad de \( X \).
    Por lo tanto, \( f \) debe tomar todos los valores entre \( f(x) \) y \( f(y) \).
\end{proof}
...
\end{ejercicio}
\begin{proof}
    Supongamos por el absurdo que \( Y \cap \partial A = \varnothing \). Sabemos que \( \Int{A} \subseteq A \subseteq \cl{A} \) y, en consecuencia, \( A^c \subseteq {\Int{A}}^c \).
    Luego, como \( \partial A = \cl{A} \setminus \Int{A} \) se tiene que: \[
        Y \cap \partial A = Y \cap \cl{A} \cap {\Int{A}}^c = \varnothing
    \]
    Como \( A \subseteq \cl{A} \) y \( A \cap Y \neq \varnothing \implies \cl{A} \cap Y \neq \varnothing \), análogamente se tiene que \( {\Int{A}}^c \cap Y \neq \varnothing \). Por lo tanto
    debe ser \( \partial A = \varnothing \), lo que implica por definición que \( \cl{A} = \Int{A} \therefore A \) es clopen e \( Y \) no es conexo.
\end{proof}
...
\end{ejercicio}
\begin{proof}
    La idea para armar el contraejemplo es aprovecharnos del hecho de que los \( F_i \) no son compactos y por ello podemos hacer la siguiente construcción: Comencemos con
    \( F_1 = [-2, 2] \times \R \) y para cada \( n \geq 2 \) consideremos \( F_n = ([-2, 2] \times \R) \setminus ((-1, 1) \times (-n, n)) \). Entonces, cada \( F_n \) es cerrado y
    conexo, pero \( \bigcap_{n \geq 1} F_n = ([-2, 2] \times \R) \setminus ((-1, 1) \times \R) \) que es la unión de dos rectángulos disjuntos y, por lo tanto, no es conexo.
\end{proof}
...
\end{ejercicio}
\begin{proof}
    Sea \( A \) un conjunto arcoconexo y \( f : A \to Y \) una función continua. Para cada \( x, y \in A \) existe un arco \( \alpha : [0, 1] \to A \) tal que \( \alpha(0) = x \) y \( \alpha(1) = y \).
    Entonces, la composición \( f \circ \alpha : [0, 1] \to Y \) es un arco en \( Y \) que conecta a \( f(x) \) con \( f(y) \). Por lo tanto, para cada par de puntos en \( f(A) \), existe un arco que los conecta dentro de \( f(A) \), lo que implica que \( f(A) \) es arcoconexo.  \end{proof}
