...
\begin{ejercicio}{14}
    Sea \(X\) un ET y para cada \(n \in \N\) sea \(f_n : X \to \R\) una función continua. Definimos
    \[
        F : X \to \R^{\N}, \qquad F(x) = (f_1(x), f_2(x), \dots).
    \]
    \begin{enumerate}
        \item Demostrar que \(F\) es continua cuando \(\R^{\N}\) lleva la topología producto.
        \item Aplicar el punto anterior al caso \(f_n = f\) para una misma función continua \(f : X \to \R\).
        \item Analizar si la conclusión del punto (a) sigue siendo cierta si en \(\R^{\N}\) se considera la topología caja. En caso negativo, dar un contraejemplo explícito.
    \end{enumerate}
\end{ejercicio}

\begin{ejercicio}{15}
    Sea \(X\) un ET Hausdorff y \(K \subseteq X\) compacto.
    \begin{enumerate}
...
\begin{ejercicio}{16}
    Sea \(X\) un ET compacto y \(\mathcal{F}\) una familia de cerrados de \(X\).
    \begin{enumerate}
