...
\begin{proof}
    ...
    \( U_x \subseteq \R \) abierto para cada \( x \in F \), quiero encontrar una función \( g \in C(X, \R) \) tal que \( g \in U \). Dado que \( X \) es CR, para cada \( x \in F \) si considero el abierto \( U = {(F \setminus \{ x \})}^c \in \tau \)
    entonces existe una función continua \( f_x : X \to [0, 1] \) tal que \( f_x(x) = 1 \) y \( f_x(y) = 0 \) para todo \( y \notin U \). Entonces, definamos la función \( g : X \to \R \) dada por
    \[
